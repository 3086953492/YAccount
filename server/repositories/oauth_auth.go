...
func GetOAuthAuthorizationCodeByCode(code, clientID string, used bool) error {
	if err := global.DB.Where("code = ? AND client_id = ? AND used = ?", code, clientID, used).First(&models.OAuthAuthorizationCode{}).Error; err != nil {
		return err
	}
	return nil
}
...
func UpdateOAuthAuthorizationCodeUsed(id uint, used bool) error {
	if err := global.DB.Model(&models.OAuthAuthorizationCode{}).Where("id = ?", id).Update("used", used).Error; err != nil {
		return err
	}
	return nil
}
...
func GetOAuthAccessTokenByRefreshToken(refreshToken, clientID string, revoked bool) error {
	if err := global.DB.Where("refresh_token = ? AND client_id = ? AND revoked = ?", refreshToken, clientID, revoked).First(&models.OAuthAccessToken{}).Error; err != nil {
		return err
	}
	return nil
}
...
func UpdateOAuthAccessTokenRevoked(id uint, revoked bool) error {
	if err := global.DB.Model(&models.OAuthAccessToken{}).Where("id = ?", id).Update("revoked", revoked).Error; err != nil {
		return err
	}
	return nil
}
